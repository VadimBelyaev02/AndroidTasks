...
class ShipGenerator(private val context: Context) {
    ...
    private val tunnel = Tunnel(context)
    private var numberOfGeneratedShips: Long = 0
    private val coroutineScope = CoroutineScope(Dispatchers.Default)
    private val dock = Dock(context)
    ...
    suspend fun generateShip(label: TextView) {
        ...
        val deferred = coroutineScope.async {
            val res = tunnel.goThroughTunnel(ship, label)
            res
        }

        val newShip = deferred.await()
        dock.loadShip(newShip, label)
